	...
	public List<SearchInquiryResponse> searchInquiry(SearchRequest request) throws Exception {
		...
		try {
			...
			if (!StringUtils.isEmpty(request.getHouseOwner()) && !StringUtils.isEmpty(request.getCity())) {
				String queryStr = "Select home.HomeID as houseId, home.owner as houseOwner,home.FloorSpace as floorSpace, home.Floors as floors,"
						+ " home.Bathrooms as bathrooms, home.LandSize as landSize,\r\n"
				...
				String queryStr = "Select DISTINCT(home.HomeID) as houseId, ho.OwnerFName as ownerFirstName, ho.OwnerLName as ownerLastName,"
						+ "home.YearConstructed as yearConstructed, home.HomeType as houseType, home.availabilityStatus as status, home.rate as price\r\n"
				...
				String queryStr = "Select DISTINCT(home.HomeID) as houseId, ho.OwnerFName as ownerFirstName, ho.OwnerLName as ownerLastName,"
						+ " listing.price, home.HomeType as houseType, listing.status,home.YearConstructed as yearConstructed \r\n"
				...
				String queryStr = "SELECT ap.MakeYear as makeYear,ap.ApplianceName as applianceName,h.owner as houseOwner, h.homeType as houseType, "
						+ "h.yearConstructed as yearConstructed, h.rate as price FROM appliances ap, home h \r\n"
				...
				String queryStr = "Select  HO.OwnerFName as ownerFirstName, HO.OwnerLName as ownerLastName,Home.homeType as houseType,"
						+ "Home.yearConstructed as yearConstructed, Address.address, Address.city, Address.state, Address.zip as zipCode,Listing.price, Listing.Status\r\n"
			...
			// earned is on the purchased price of a home he/she sells.
				...
				String queryStr = "SELECT Agent.AgentID, AgentFName as agentFirstName, AgentLName as agentLastName, "
						+ "round(sum(listing.price * (listing.CommissionRate/100)),2) AS totalCommission \r\n"
				...
				// TO DO
				String queryStr = "select distinct ho.ownerID ,ho.OwnerFName as ownerFirstName, ho.OwnerLName as ownerLastName, "
						+ "home.homeType as houseType, home.homeID, home.YearConstructed as yearConstructed,"
				...
				// TO DO
				String queryStr = "SELECT distinct ho.OwnerFName as ownerFirstName,ho.OwnerLName as ownerLastName,ls.price, h.homeType as houseType,"
						+ " h.yearConstructed as yearConstructed, ad.address, ad.city, ad.state, ad.zip as zipCode, h.availabilityStatus as status"
						+ " FROM homeowners ho\r\n"
				...
				// TO DO
				String queryStr = "SELECT distinct home.hometype as houseType, home.HomeID, home.YearConstructed as yearConstructed,"
						+ " listing.price, home.owner as houseOwner, address.address, address.city, address.state, address.zip as zipCode\r\n"
						+ "					FROM Home home\r\n"
